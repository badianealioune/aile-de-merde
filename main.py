...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")
        await self.highrise.walk_to(Position(14.5 , 0.0 , 2.5 , "FrontRight"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala")   
        await self.highrise.chat(f"Bienvenue {user.username} mon honorable esclave❤️.Pour danser utilise de 1 à 95.Ecris help pour voir la liste des commandes")
      
        await self.highrise.send_emote("emote-boxer")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.startswith("SPECTRO06"):
          if user.username == "ALIOUNE_":
            roomUsers = (await self.highrise.get_room_users()).content
            for roomUser, _ in roomUsers:
    ...
    async def on_user_leave(self, user: User) -> None:
        ...
        await self.highrise.chat(f"Au revoir @{user.username} tu vas manquer l'air frais❤️")
        await self.highrise.send_emote("dance-employee")